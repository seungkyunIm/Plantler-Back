...
public interface FreeBoardMapper {
	...
	@Select({"  <script>"
			...
			+ " WHERE pb.category_id IN (100,101,102,103) AND pb.board_title like '%${query}%' "
			+ "	<if test='category_id != 0'> AND pb.category_id = ${category_id} </if> "
			+ " ORDER BY 1 DESC LIMIT ${page}, ${size}"
			+ "</script>"})
	public List<FreeBoardDTO> findAll(RequestBoardDTO requestBoardDTO);
	...
	@Select("<script> "
			...
			+ "	INNER JOIN pl_category AS pc "
			+ "	ON (pb.category_id = pc.category_id) "
			+ "	WHERE pb.category_id >= 100 AND board_title like '%${query}%' "
			+ "	 <if test= 'category_id != 0'> AND pb.category_id = ${category_id} </if> "
//			+ "	ORDER BY 1 DESC LIMIT ${page}, ${size} "
			+ "	</script>")
	...
	public int saveBoard(FreeBoardDTO freeBoardDTO);
	...
	public FreeBoardDTO findByBoardNo(int board_no);
	...
	public List<FreeBoardDTO> FreeTop10ByBoardLikes(int limit);	//like컬럼이 없어져서 임의로 board_count
	...
	public int updateBoard(FreeBoardDTO freeBoardDTO);
	...
	public KhBoardLikeDTO khboardFindByActive(KhBoardLikeDTO khBoardLikeDTO);
	
	@Insert("INSERT INTO pl_board_likes (board_no, user_no) VALUE (#{board_no}, #{user_no})")
	public int khboardAddActive(KhBoardLikeDTO khBoardLikeDTO);
	
	@Delete("DELETE FROM pl_board_likes WHERE board_no = #{board_no} AND user_no = #{user_no}")
    public int khboardDeleteActive(KhBoardLikeDTO khBoardLikeDTO);
    
    @Select("SELECT COUNT(*) AS cnt FROM pl_board_likes WHERE board_no = #{board_no}")
    public int khboardFindByCount(KhBoardLikeDTO khBoardLikeDTO);
    ...
    //댓글-----------------------------------------------------------------------------------------------
  	...
  	// 카테고리별 게시글 목록 불러오기
  	@Select("SELECT * FROM pl_category WHERE category_id BETWEEN #{start} AND #{end}")
  	public List<CategoryDTO> findByCategory(CategoryParamDTO param);
