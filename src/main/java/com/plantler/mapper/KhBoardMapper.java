...
public interface KhBoardMapper {
	...
	// 전체 게시글 리스트
	@Select({"<script> "
			+ "  SELECT pb.*, pu.user_nick, pc.category_name, pbl.`active` AS board_like, "
			...
			+ " WHERE pb.category_id IN (1,2,3,4,5,6) AND pb.board_title like '%${query}%' "
			+ " <if test='category_id != 0'> AND pb.category_id = ${category_id} </if> "
			+ " ORDER BY 1 DESC LIMIT ${page}, ${size} " 
			+ " </script>"})
	public List<KhBoardDTO> findAll(Map<String, Object> paramMap);
	...
	@Select({"<script> "
			...
			+ "INNER JOIN pl_category AS pc "
			+ " ON (pb.category_id = pc.category_id) "
			+ "WHERE pb.category_id IN (1, 2, 3, 4, 5, 6) AND board_title like '%${query}%' "
			+ " <if test= 'category_id != 0'> AND pb.category_id = ${category_id} </if> "
//			+ "ORDER BY 1 DESC LIMIT ${page}, ${size} "
			+ "</script>"})
	public int findAllTotal(Map<String, Object> paramMap);
	...
	public List<KhBoardDTO> KhTop10ByBoardLikes(int limit);	//like컬럼이 없어져서 임의로 board_count
